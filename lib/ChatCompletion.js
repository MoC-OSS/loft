...
class ChatCompletion {
    ...
    llmApiCallQueue;
    llmApiCallWorker;
    constructor(cfg, sms, ps, hs, connection, errorHandler) {
        this.cfg = cfg;
        this.sms = sms;
        this.ps = ps;
        this.hs = hs;
    ...
    static async createInstance(cfg, sms, ps, hs, connection, errorHandler) {
        l.info('Creating ChatCompletion instance...');
        const instance = new ChatCompletion(cfg, sms, ps, hs, connection, errorHandler);
    ...
    async chatCompletionBeginProcessor(job) {
        ...
        try {
            ...
            const { messages: processedMessages, ctx: InputContext } = await this.llmIOManager.executeInputMiddlewareChain(job.data);
            ...
            if (await this.hs.isExists(sessionId, systemMessageName)) {
                ...
                if (session.messageAccumulator !== null || session.lastError !== null) {
                    ...
                    await this.hs.appendMessagesToAccumulator(sessionId, systemMessageName, processedMessages, session, InputContext);
              ...
              And after that, all messages from session.messageAccumulator will be added to session.messages and chat flow will be continued.`);
                    ...
                    await this.hs.appendMessages(sessionId, systemMessageName, processedMessages, InputContext);
                }
            }
            else {
                ...
                await this.hs.createSession(sessionId, systemMessageName, systemMessage, model, modelPreset, examples, [...processedMessages], InputContext);
            }
