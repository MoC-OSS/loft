...
class ChatCompletion {
    ...
    async callFunction(chatCompletionResult, ctx) {
        ...
        if (fnName &&
            ...
            const newSession = await this.hs.getSession(sessionId, systemMessageName);
            const jobKey = (0, helpers_1.sanitizeAndValidateRedisKey)(`app:${this.cfg.appName}:api_type:chat_completion:function:bullmq_job:job_name:chat_completion_call_processor:intent:set_function_result:session:${sessionId}:system_message:${systemMessageName}:time:${this.getTimastamp()}`);
