...
class ChatCompletion {
    ...
    async injectPromptAndSend(promptName, session, messages, promptRole = 'user') {
    ...
    async callRetry(sessionId, systemMessageName) {
        ...
        if (session.messages[session.messages.length - 1].role !==
            openai_1.ChatCompletionRequestMessageRoleEnum.User ||
            session.messages[session.messages.length - 1].role !==
                openai_1.ChatCompletionRequestMessageRoleEnum.Function) {
    ...
    async chatCompletionCallProcessor(job) {
        let session = new Session_1.Session(this.hs, job.data);
        ...
        try {
            ...
            */
            ...
            if (modelPreset.function_call === 'auto') {
                l.info(`${logPrefix} modelPreset.function_call: auto, calling function...`);
                if (await this.callFunction(chatCompletion.data, outputContext)) {
                    l.info(`${logPrefix} function called, job canceled.`);
                    return;
                }
                else
                    l.info(`${logPrefix} function not called. Continue...`);
            }
