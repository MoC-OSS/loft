...
class SessionStorage {
    ...
    async createSession(sessionId, systemMessageName, modelPreset, messages) {
        const sessionKey = this.getChatCompletionSessionKey(sessionId, systemMessageName);
        ...
        const session = new Session_1.Session(this, {
            ...
            systemMessageName,
            modelPreset,
            messages: new ChatHistory_1.ChatHistory(sessionId, systemMessageName, ...messages),
            lastMessageByRole: {
                user: null,
                assistant: null,
                system: messages[0],
    ...
    async replaceLastUserMessage(sessionId, systemMessageName, newMessage, role = 'user') {
        l.info(`Replace last user message in session ${sessionId}, systemMessageName: ${systemMessageName}`);
        const session = await this.getSession(sessionId, systemMessageName);
    ...
    async save(session) {
        ...
        if ((0, helpers_1.deepEqual)(existingSession.ctx, session.ctx) &&
            (0, helpers_1.deepEqual)(existingSession.messages, session.messages)) {
