...
class LlmOrchestrator {
    ...
    async injectPromptAndSend(promptName, sessionId, systemMessageName, message, promptRole = 'user', messageRole = 'user') {
        ...
        const jobKey = (0, helpers_1.sanitizeAndValidateRedisKey)(`app:${this.cfg.appName}:api_type:chat_completion:function:bullmq_job:job_name:chat_completion_call_processor:intent:prompt_injection:session:${sessionId}:system_message:${systemMessageName}:time:${this.getTimastamp()}`);
     ...
     */
    async callAgain(sessionId, systemMessageName, message, role = 'user') {
        try {
            if (!(await this.hs.isExists(sessionId, systemMessageName)))
                throw new Error("inject prompt failed: chatId doesn't exist");
            const processedInputContext = await this.llmIOManager.executeInputMiddlewareChain({
    ...
    async chatCompletionBeginProcessor(job) {
        try {
            ...
            if (await this.hs.isExists(chatId, systemMessageName)) {
                ...
                await this.hs.createSession(chatId, systemMessageName, modelPreset, systemMessage);
                await this.hs.updateMessages(chatId, systemMessageName, newMessage);
            }
            const chatSession = await this.hs.getSession(chatId, systemMessageName);
            const jobKey = (0, helpers_1.sanitizeAndValidateRedisKey)(`app:${this.cfg.appName}:api_type:chat_completion:function:bullmq_job:job_name:chat_completion_call_processor:intent:continue_chat_processing:session:${chatSession.sessionId}:system_message:${systemMessageName}:time:${this.getTimastamp()}`);
